$(document).ready(function () {
    var mapElement = $('#main-map').mapQuery();
    var myMap = mapElement.data('mapQuery');
    myMap.olMap.projection = 'EPSG:900913';
    myMap.olMap.maxExtent = new OpenLayers.Bounds( -128 * 156543.0339, -128 * 156543.0339, 128 * 156543.0339, 128 * 156543.0339); 
    myMap.olMap.sphericalMercator = true;
    myMap.olMap.units = 'm';
    myMap.olMap.maxResolution= 156543.0339;
    ...
    myMap.layer({
        id: "googleterrain",
        label: "Google Terrein",
        type: "google",
        groupselector: "#baselayer",
        legendurl: "http://maps.gstatic.com/intl/en_ALL/mapfiles/poweredby.png",
        layers: G_PHYSICAL_MAP,
        isBaseLayer: true,
        sphericalMercator: true
    });
    myMap.layer({
    ...
    myMap.layer({
        id: "googlefoto",
        label: "Google Luchtfoto's",
        type: "google",
        groupselector: "#baselayer",
        legendurl: "http://maps.gstatic.com/intl/en_ALL/mapfiles/poweredby.png",
        layers: G_SATELLITE_MAP,
        isBaseLayer: true,
        sphericalMercator: true
    });
    ...
    myMap.layer({
        id: 'onegeonorth',
        type: 'TMS',
        label: 'OneGeology North',        
        layers: 'north',
        url:'http://mapproxy.geodan.nl/tms/',
        file: 'png',
        isBaseLayer: false,
        zoomOffset:-1,
        buffer:0,
        groupselector: "onegeology",
        visibility: false
    });
    myMap.layer({
        id: 'onegeosouth',
        type: 'TMS',
        label: 'OneGeology South',        
        layers: 'south',
        url:'http://mapproxy.geodan.nl/tms/',
        file: 'png',
        isBaseLayer: false,
    ...
    myMap.layer({
        id: 'geology-spain',
        type: 'WMS',
        label: 'Spain - Geological Units',
        url: 'http://mapproxy.geodan.nl/service',
        layers: 'spain_geological_units',
        groupselector: "geology",
        visibility: false
    });
    myMap.layer({
        id: 'geology-portugal',
        type: 'WMS',
        label: 'Portugal - Geological Units',
        url: 'http://mapproxy.geodan.nl/service',
        layers: 'portugal_geological_units',
        groupselector: "geology",
        visibility: false
    });
    //myMap.olMap.setCenter(new OpenLayers.LonLat(12,53).transform(new OpenLayers.Projection("EPSG:4326"), sphericalMercator),4);
    ...
    myMap.layer('Google', {
        ...
        'label' : "Google Terrein",
        'layers' : G_PHYSICAL_MAP,
        'layerOptions' : { isBaseLayer: true, sphericalMercator: true, type: G_PHYSICAL_MAP }
    });
    */
    ...
    // add coordinate display for mouseposition 
//  $("#mouse-position").mapquerymouseposition (myMap.olMap);
    
    // add scale bar
    //$("#scale-info").mapqueryscalebar (myMap.olMap,{ labelScale: ""});
    
    // add overview map
    //$("#main-map").mapqueryoverviewmap (myMap.olMap, { title: "Overview"});
    
    // add legend
    $("#referencedata").mqLegendToc(mapElement);
    $("#baselayers").mqBaseLayerToc(mapElement);
    $("#commoditylist").mqCommodityToc(mapElement);
    // add feature info handling
//  $("#main-map").mapqueryuifeatureinfo ({ title: "Information"});

    // add search form
//  $("#geo-search").mapquerygeosearch (myMap.olMap);

    // add permalink catcher + builder
//  $("#permalinkbuilder").mapquerypermalink("#main-map", "#map-layers");

    // add map browser history
//  $("#main-map").mapqueryhistory();
        
    // map overview-toggle animate close transition
//  $("#mapqueryoverviewmap1").bind( "dialogbeforeclose", function(event, ui) {
//      $("#mapqueryoverviewmap1").effect("transfer", { to: "#overview-toggle", className: "ui-effects-transfer" }, 500);
//  });
    
    // map overview-toggle click handler
    $("#overview-toggle").click(function () {
        $overviewmap = $("#mapqueryoverviewmap1");
        if ($overviewmap.dialog('isOpen')) {
    ...
    //TODO: move above to a proper place
